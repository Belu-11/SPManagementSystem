...
namespace Plugins.DataStore.SQL
{
    internal class ProductSQLRepository : IProductRepository
    {
        ...
        public ProductSQLRepository(StoreContext db)
        {
            this.db = db;
        }
        ...
        public IEnumerable<Product> GetProducts(bool loadCategory = false)
        {
            if(loadCategory)
                return db.Products.
                    Include(x => x.Category).OrderBy(x => x.CategoryId).ToList();
            else
                return db.Products.OrderBy(x => x.CategoryId).ToList();

        }

        public IEnumerable<Product> GetProductsByCategoryId(int caategoryId)
        {
            return db.Products.Where(x => x.CategoryId == caategoryId).ToList();
        }
